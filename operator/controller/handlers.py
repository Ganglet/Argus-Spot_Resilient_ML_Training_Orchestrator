...
def on_create(spec, name, namespace, patch, **kwargs):
    ...
    # Delegate Pod creation to our scheduler logic
    scheduler.create_pod(name, namespace, spec)
...
def reconcile(spec, name, namespace, status, patch, **kwargs):
    ...
    try:
        node_name = scheduler.get_pod_node(f"{name}-pod", namespace)
        if node_name:
            scheduler.cordon_node(node_name)
        scheduler.reschedule_pod(f"{name}-pod", namespace, fallback_types)
        
        # We also need to re-create the pod since we're not using a Deployment/Job wrapper
        logger.info(f"[RECONCILE] Recreating pod '{name}-pod' after eviction")
        scheduler.create_pod(name, namespace, spec)
