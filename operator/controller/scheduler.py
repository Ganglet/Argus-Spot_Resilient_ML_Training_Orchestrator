...
def create_pod(job_name: str, namespace: str, spec: dict) -> None:
    """
    Creates a new Pod for the SpotResilientJob.
    """
    v1 = client.CoreV1Api()
    
    # Parse S3_BUCKET and S3_PREFIX from checkpointPath
    checkpoint_path = spec["checkpointPath"].replace("s3://", "")
    bucket, prefix = checkpoint_path.split("/", 1)
        ...
        "spec": {
            "containers": [{
                ...
                "env": [
                    ...
                    {"name": "S3_PREFIX", "value": f"{prefix}/latest_checkpoint.pt"},
    ...
    try:
        ...
        logger.info(f"[SCHEDULER] Created pod '{job_name}-pod' for job '{job_name}'")
    except client.exceptions.ApiException as e:
        if e.status == 409:
            logger.warning(f"[SCHEDULER] Pod '{job_name}-pod' already exists.")
        else:
            raise
